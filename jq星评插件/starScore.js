	...
	var defaults={
		fen_d:14,  // 每个a的宽度
		ScoreGrade:10,  // a的个数
		types:["1","1","1.5","2","2.5","3","3.5","4","4.5","5"],
		       nameScore:"score",
		       parent:"star_score",
		       attitude:"attitude"};
        ...
        show(index,$(this))
		  ...
		  $(this).mouseleave(function(){    // mouseleave事件
		  	if(now_cli>=0){
                ...
                if(scor==0.5){
                    startParent.find("a").eq(1).addClass("clibg");
                    countScore.text(1)
                }else{
                ...
                startParent.find("a").eq(9).addClass("clibg");
		  		if(countScore){
		  			countScore.text("5")
		  		}
		  	}
      ...
      // show方法
		  function show(num,obj){
              ...
              if(scor==0.5){
                  obj.next().addClass("clibg")
                  countScore.text(1);                  // 显示评分
              }else {
		    ...
		    object.find("a").attr("title",atu);  // 显示用户态度
		  }
		};